...
def generate_ad_file(cfg: example_setups.setup, path: str, controller: rf_helpers.rf_controller, filename: str) -> int:
    ...
    pathlib.Path(path, "gen_info").mkdir(parents=True, exist_ok=True)
    ...
    try:
        with open(pathlib.Path(path, adfile), 'w') as fp:
            for s in samples:
                fp.write("{:d}\n".format(s))
        with open(pathlib.Path(path, "gen_info", "{}.info".format(filename)), 'w') as fp:
            fp.write("Generated Stimuli file with frequency: {} MHz @ {} dBFS\n".format(
                controller.get_real_frequency(freq) / 1e6, amp))
            fp.write("The requested frequency was {} MHz, a difference of {} Hz\n".format(freq / 1e6,
                                                                                          controller.get_real_frequency(
                                                                                              freq) - freq))
    except IOError as e:
        print("Couln't create file")
        raise e
    plot = pathlib.Path(path, "plots", filename + ".png")
